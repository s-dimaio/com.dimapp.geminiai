...
function getTools() {
    return [
        ...
        },
        {
            ...
            description: `
                Manage device firmware updates (for Matter nodes).

                **Use this to:**
                - List all supported devices and check which ones have a firmware update available (using action='list').
                - Check if a specific device has an update available (using action='check').
                - Trigger the installation of a firmware update on a specific device (using action='install').

                ### IMPORTANT: Target Limits
                * **IMPORTANT**: Programmatic updates via this tool are currently supported only for Matter devices. If requested for other device classes, you may still run the tool and use its response to explain the limitation to the user.
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    softwareVersion: {
                        type: Type.INTEGER,
                        description: "Required for Matter node 'install' action: the target software version number."
                    }
                },
                required: ['action']
            }
