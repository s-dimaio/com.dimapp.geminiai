...
function getTools() {
    return [
        ...
        },
        {
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    trigger: {
                        type: Type.OBJECT,
                        description: "Trigger card configuration. Required for 'create'. Format: { id: string, args: object }. The 'id' is the full trigger card ID as returned by discover_flow_cards (e.g. 'homey:manager:cron:time_exactly'). Do NOT include a 'uri' field. CRITICAL: The 'args' property MUST strictly be a native JSON object (e.g., {\"threshold\": 5}). NEVER stringify it, never wrap it in quotes, and never pass it as a JSON array. When updating a flow and changing the trigger card, you MUST completely replace 'args' with ONLY the exact arguments accepted by the new trigger card schema. Never carry over arguments from the previous trigger."
                    },
                    conditions: {
                        type: Type.ARRAY,
                        description: "Array of condition card configurations. Can be empty []. Each: { id: string, args: object, group: string, inverted?: boolean }. The 'group' property is MANDATORY (usually set to 'group1') to prevent rendering bugs in the Homey UI. Do NOT include a 'uri' field. CRITICAL: The 'args' property inside each condition card MUST strictly be a native JSON object (e.g., {\"threshold\": 5}). NEVER stringify it, and never pass it as a JSON array. When updating a flow and replacing a condition card, you MUST completely replace its 'args' object to match the new card's schema as returned by discover_flow_cards.",
                        items: { type: Type.OBJECT }
                    },
                    actions: {
                        type: Type.ARRAY,
                        description: "Array of action card configurations. Must contain at least one item for 'create'. Each action MUST have exactly these fields: { id: string, args: object, group: 'then', delay: <DelayObject|null>, duration: null }. DelayObject format is { enabled: true, number: '10', multiplier: 1 } (multiplier is strictly 1 for seconds, 60 for minutes. If the user specifies hours, convert them to minutes, e.g. 1.5 hours = 90 minutes). Set delay to null if no delay is requested. The 'id' is the full action card ID as returned by discover_flow_cards (e.g. 'homey:device:UUID:off'). Do NOT include a 'uri' field. CRITICAL: The 'args' property inside each action card MUST strictly be a native JSON object (e.g., {\"message\": \"hello\"}). NEVER stringify it, and never pass it as a JSON array. When updating a flow and changing or replacing an action card, you MUST discard the old 'args' object completely and populate 'args' with ONLY the exact arguments accepted by the new card schema. NEVER carry over parameters (like 'message') from the previous action card.",
                        items: { type: Type.OBJECT }
                    },
                    enabled: {
                        type: Type.BOOLEAN,
        ...
        },
        {
            ...
            description: `
                Create, update, or delete an Advanced Homey Pro Flow (graph-based DAG canvas automation).
                ...
                **How coordinates (x, y) are handled:**
                - You do NOT need to specify 'x' and 'y' layout coordinates! The system automatically computes them using a beautiful Breadth-First-Search layout engine.
                - If you want custom positioning, you may specify 'x' and 'y' coordinates on individual cards.

                **Cards structure Map format:**
                - The 'cards' object is a map where keys are arbitrary UUIDs you generate (e.g. 'ae825c34-0f14-4d68-84a1-16c9d8bddbb2') and values are card node objects:
                  - 'ownerUri': string (e.g. 'homey:device:<uuid>' or 'homey:app:com.dimapp.geminiai' or 'homey:manager:cron').
                  - 'id': string (exact card ID, e.g. 'homey:device:<uuid>:onoff' or 'homey:manager:cron:time_exactly').
                  - 'type': string ('trigger', 'condition', 'action', 'any', 'all', 'delay').
                  - 'input': optional array of string (ONLY required for 'all' cards, maps preceding trigger/condition outputs, e.g. ["<parentCardUuid>::outputSuccess"]).
                  - 'args': object (arguments required by the card, e.g. { 'time': '08:00' }). **CRITICAL:** This MUST be an actual nested JSON object. NEVER pass 'args' as a stringified JSON string (do NOT wrap the entire object in quotes or use backslash-escaped quotes inside it). NEVER pass 'args' as a JSON array (e.g. do NOT use [] or [{...}]). If the card requires no arguments, leave 'args' as an empty object {} or omit it. For 'all' and 'any' cards, 'args' is not used (leave omitted or set to {}).
                  - 'delay': optional number (delay in seconds).
                  - 'duration': optional number (duration in seconds).
                  - 'droptoken': optional string (only for condition cards, filters by preceding card's output, e.g. 'action::<prevCardUuid>::success').
                  - 'outputSuccess': optional array of string (UUID keys of target cards to trigger on success).
                  - 'outputError': optional array of string (UUID keys of target cards to trigger on error).
                  - 'outputTrue': optional array of string (UUID keys of target cards to trigger on true).
                  - 'outputFalse': optional array of string (UUID keys of target cards to trigger on false).`,
            inputSchema: {
                type: Type.OBJECT,
                properties: {
                    action: {
                        type: Type.STRING,
                        description: "Operation to perform: 'create' (new advanced flow), 'update' (modify existing advanced flow), 'delete' (remove existing advanced flow).",
                        enum: ['create', 'update', 'delete']
                    },
                    name: {
                        type: Type.STRING,
                    ...
                    cards: {
                        type: Type.OBJECT,
                        description: "Map of card node objects. Keys are UUIDs you generate. Required for 'create', optional for 'update'. Values must match the Card Node structure."
                    },
                    enabled: {
                        type: Type.BOOLEAN,
                        description: "Optional. Whether the flow should be enabled. Defaults to true for 'create'. Use in 'update' to enable/disable."
