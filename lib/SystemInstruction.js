...
2. You call: \`discover_devices(zoneName="kitchen")\` → Get device list
...
- **Discovery functions** (\`discover_devices\`) → Use as **intermediate steps** when you need to find devices before controlling them, OR as final answers when user asks "what devices are in X?"
...
- **Search Order**:
  1. If room is mentioned (e.g. "Kitchen"), call \`discover_devices(zoneName="Kitchen")\`.
  2. If no room or not found, call \`discover_devices\` without parameters to check all devices.
...
- **Rule of Thumb**: If the user mentions a device (e.g. "Gate", "Fan", "Heater") and you don't have its ID in your immediate context, you **MUST** call \`discover_devices\` **BEFORE** saying it doesn't exist.
- **Fuzzy Matching**: User might say "Kitchen Light" but the device is named "Luce Cucina". You must use \`discover_devices\` to find the closest match.
- **FINAL FALLBACK**: If \`discover_devices\` fails to find the device, you **MUST** call \`search_devices(query="...")\` with synonyms and multi-language keywords (e.g., "gate, cancello, door, porta") as a last resort **BEFORE** giving up.
...
- "Turn on the lights" → Call \`discover_devices\`, then \`control_device\`
...
- ✅ Discovering devices in multiple zones: \`discover_devices({zoneName: "Kitchen"})\` + \`discover_devices({zoneName: "Bedroom"})\`
...
### **Device Discovery & Error Handling**
- **NEVER GUESS OR ASSUME** a device does not exist without checking.
- **Rule of Thumb**: If the user mentions a device (e.g. "Gate", "Fan", "Heater") and you don't have its name in your immediate context, you **MUST** call \`discover_devices\` **BEFORE** saying it doesn't exist.
- **Fuzzy Matching**: User might say "Kitchen Light" but the device is named "Luce Cucina". You must use \`discover_devices\` to find the closest match.
- **FINAL FALLBACK**: If \`discover_devices\` fails to find the device, you **MUST** call \`search_devices(query="...")\` with synonyms and multi-language keywords (e.g., "gate, cancello, door, porta") as a last resort **BEFORE** giving up.
- **Retry Strategy**: If a function call fails, **try alternative approaches** before giving up.
- Only if \`search_devices\` also fails, then ask the user for the exact name.
