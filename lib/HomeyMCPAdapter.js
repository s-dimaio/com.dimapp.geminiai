...
class HomeyMCPAdapter {
  ...
  async listDevicesByClass(deviceClass) {
    ...
    // Filter devices by class OR virtualClass
    // virtualClass is set when user configures "What's plugged in?" for sockets/switches
    const filteredDevices = Object.values(devices).filter(d => {
      const deviceClass = d.class?.toLowerCase();
      const virtualClass = d.virtualClass?.toLowerCase();
      return deviceClass === normalizedClass || virtualClass === normalizedClass;
    });
  ...
  */
  ...
  async getDevicesStatusByClass(deviceClass, zoneName = null) {
    try {
      ...
      // Filter by class OR virtualClass
      // virtualClass is set when user configures "What's plugged in?" for sockets/switches
      let classDevices = Object.values(devices).filter(d =>
        d.class === deviceClass || d.virtualClass === deviceClass
      );
  ...
  _computeZoneSummary(devices) {
    ...
    devices.forEach(d => {
      ...
      if (!summary.byClass[effectiveClass]) {
        summary.byClass[effectiveClass] = { total: 0, on: 0, off: 0 };
      }

      summary.byClass[effectiveClass].total++;

      if (d.capabilitiesObj?.onoff?.value === true) {
        summary.byClass[effectiveClass].on++;
      } else if (d.capabilitiesObj?.onoff) {
        summary.byClass[effectiveClass].off++;
      }
    });
