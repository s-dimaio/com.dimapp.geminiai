  ...
  async manageFlow(args) {
    ...
    try {
      ...
      if (action === 'create') {
        ...
        const flowObjJson = JSON.stringify(flowObj).replace(/\\/g, '\\\\').replace(/"/g, '\\"');
        const created = await this._executeHomeyScript(`return await Homey.flow.createFlow({ flow: JSON.parse("${flowObjJson}") });`);
        return {
          success: true,
      ...
      if (action === 'update') {
        ...
        const flowObjJson = JSON.stringify(updatedFlowObj).replace(/\\/g, '\\\\').replace(/"/g, '\\"');
        const updated = await this._executeHomeyScript(`return await Homey.flow.updateFlow({ id: "${flow.id}", flow: JSON.parse("${flowObjJson}") });`);
        return {
          success: true,
