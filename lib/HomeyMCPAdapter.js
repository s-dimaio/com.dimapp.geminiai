...
class HomeyMCPAdapter {
  ...
  async callTool(name, args) {
    try {
      ...
      switch (name) {
        ...
        // Legacy alias: kept for backward compatibility with any cached context still referencing the old name.
        // TODO: Remove in a future release — the Gemini context cache TTL is a few minutes and resets on app reinstall,
        //       so this alias has no long-term value. Update the system prompt and tool schema are already aligned.
        ...
        case 'manage_device_firmware':
          return await this.manageDeviceFirmware(args);
        default:
   ...
   * @throws Never — errors are returned as `{ success: false, error }`.
   * @example
   * const result = await adapter.runActionCard('Living Room Light', 'homey:device:123:onoff', { value: true });
   */
  ...
  async manageFlow(args) {
    ...
    try {
      ...
      if (action === 'restore') {
        ...
        if (exists) {
          this.homey.log(`[manageFlow] Restoring existing flow id=${originalId}`);
          ...
          this.homey.log(`[manageFlow] Recreating deleted flow name="${flowToRestore.name}"`);
          const code = `return await Homey.flow.createFlow({ flow: JSON.parse(Buffer.from("${base64Str}", "base64").toString("utf8")) });`;
          restoredFlow = await this._executeHomeyScript(code);
  ...
  async manageAdvancedFlow(args) {
    ...
    // Convert cards Array (from ToolSchema) into the UUID-keyed Map dictionary format expected by the backend
    ...
    if (cards && typeof cards === 'object') {
      for (const key of Object.keys(cards)) {
        ...
        if (!card || typeof card !== 'object') {
          // Defensive guard: delete primitive keys hallucinated directly under the 'cards' root map (e.g. id, type, ownerUri)
          delete cards[key];
          continue;
        }
        ...
        // Enforce the system instruction constraint: never accept spatial coordinates
        // from the model. Stripping them here guarantees that the auto-layout engine
        // is always used to compute fresh, clean coordinates.
        delete card.x;
        delete card.y;
    ...
    try {
      ...
      if (action === 'create') {
        ...
        // Clean up action field just in case it was provided
          ...
          for (const [uuid, cardNode] of Object.entries(cards)) {
            ...
            if (cardAction === 'delete') {
              ...
              if (mergedCards[uuid]) {
                ...
                mergedCards[uuid] = cardNode; // Fallback to create if not exists
              }
      ...
      if (action === 'restore') {
        ...
        if (exists) {
          this.homey.log(`[manageAdvancedFlow] Restoring existing advanced flow id=${originalId}`);
          ...
          this.homey.log(`[manageAdvancedFlow] Recreating deleted advanced flow name="${flowToRestore.name}"`);
          const code = `return await Homey.flow.createAdvancedFlow({ advancedflow: JSON.parse(Buffer.from("${base64Str}", "base64").toString("utf8")) });`;
          restoredFlow = await this._executeHomeyScript(code);
   ...
   * Calculates dynamic column widths based on whether card nodes are standard width (e.g. actions/triggers)
   * or compact logic/control elements (e.g. START, DELAY, ALL, ANY) to ensure compact and elegant connections.
   *
   ...
   *
   * @private
   * @method _applyAdvancedFlowAutoLayout
   * @memberof HomeyMCPAdapter
   * @param {Object} cards - The map of UUID keys to card objects to layout (mandatory).
   * @returns {Object} The same cards map with calculated x and y properties where missing.
   * @example
   * // Called internally inside manageAdvancedFlow:
  ...
  _applyAdvancedFlowAutoLayout(cards) {
    ...
    // 6. Set coordinates (dynamic x spacing based on card widths, y spacing: 160px, center around y=400)
    const getCardWidth = (type) => {
      if (type === 'start') return 50;
      if (type === 'any' || type === 'all') return 90;
      if (type === 'delay') return 110;
      return 280; // Standard flow card width
    };
    ...
    const GAP_BETWEEN_CARDS = 120; // Optimal visual distance between card boundaries

    for (let i = 0; i < sortedLevels.length - 1; i++) {
      const currentLvl = sortedLevels[i];
      const nextLvl = sortedLevels[i + 1];

      // Find the maximum width among all cards in the current level
    ...
    Object.entries(levelGroups).forEach(([lvlStr, group]) => {
      ...
      const xCoord = xCoords[lvl] ?? (50 + lvl * 450); // Fallback to old behavior if somehow undefined
  ...
  /**
   * Checks or installs device firmware updates (Matter nodes, Zigbee controller).
   *
   * @public
   * @param {Object} opts - The options object.
   * @param {string} opts.action - The action to perform: 'check' or 'install' (mandatory).
   * @param {string} [opts.deviceName] - The name of the device to query/update (optional).
   * @param {string} [opts.deviceId] - The UUID of the device/node or 'zigbee' (optional).
   * @param {number} [opts.softwareVersion] - The software version to install (mandatory for Matter installation).
   * @returns {Promise<Object>} The operation result.
   * @example
   * const result = await adapter.manageDeviceFirmware({
   *   action: 'check',
   *   deviceName: 'My Matter Switch'
   * });
   */
  async manageDeviceFirmware({ action, deviceName, deviceId, softwareVersion }) {
    if (!action) {
    ...
    // 1. Action: list (Global overview scan in parallel)
    if (action === 'list') {
      try {
        const devices = await this.api.devices.getDevices();
        ...
        let matterNodes = {};
        try {
          matterNodes = await this.api.matter.getMatterNodes();
        } catch (e) {
          this.homey.log('[manageDeviceFirmware] Matter nodes not available:', e.message);
        }

        // Prepare check promises
        const promises = [];

        // Add Matter Node checks
        for (const [nodeId, node] of Object.entries(matterNodes)) {
          // Map Matter Node ID to the corresponding Homey Device(s) using data.nodeCRUDItemId.
          // Multiple endpoints may map to the same node; we prefer nodeIndex === 0 or the first match.
          ...
          promises.push(
            this.api.matter.getNodeAvailableFirmwareUpdate({ id: nodeId })
              .then(updateInfo => ({
                target: 'matter',
                id: matchedDeviceId,
                matterNodeId: nodeId,
    ...
    if (matterNodeId) {
      try {
        const matterNodes = await this.api.matter.getMatterNodes();
    ...
    // 5. Route to appropriate API Manager
    if (isMatter && matterNodeId) {
      if (action === 'check') {
        try {
          const updateInfo = await this.api.matter.getNodeAvailableFirmwareUpdate({ id: matterNodeId });
          let nodeInfo = null;
          let statusSummary = null;
          try {
            nodeInfo = await this.api.matter.getMatterNode({ id: matterNodeId });
            if (nodeInfo) {
        ...
        try {
          const result = await this.api.matter.installNodeAvailableFirmwareUpdate({ id: matterNodeId, softwareVersion });
          return {
            success: true,
            target: 'matter',
            deviceId: device.id,
  ...
  /**
   * Writes a backup of the specified flow object to the local disk if the protection
   * time window has elapsed or if no backup exists.
   *
   * @private
   * @async
   * @param {string} flowId - The unique identifier of the flow to backup (mandatory).
   * @param {Object} flowObj - The full flow data object to be backed up (mandatory).
   * @param {'standard'|'advanced'} type - The type of flow: standard or advanced (mandatory).
   * @returns {Promise<void>} Resolves when the backup operation is complete or skipped.
   * @example
   * await this._writeBackup('flow-id-123', flowObj, 'standard');
   */
    ...
    try {
      ...
      try {
        ...
        if (hoursDiff < BACKUP_PROTECTION_HOURS) {
          this.homey.log(`[Backup] Skipping backup for flow ${flowId}: a recent backup already exists (created ${hoursDiff.toFixed(1)} hours ago).`);
          return;
        }
      } catch (err) {
      ...
      this.homey.log(`[Backup] Saved backup for flow ${flowId} at ${backupPath}`);
    } catch (err) {
      this.homey.log(`[Backup] Failed to write backup for flow ${flowId}:`, err.message);
    }
  }
