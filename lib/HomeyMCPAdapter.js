 ...
 */
  ...
  async _runActionCardViaHomeyScript(cardId, deviceUri, args) {
    ...
    // Return the result value (for action cards that return tokens)
  ...
  async listDeviceActions(deviceName) {
    ...
    try {
      ...
      const formattedActions = deviceActions.map(card => ({
        ...
        tokens: card.tokens || [] // Output tokens (text, number, boolean, image)
      }));
  ...
  /**
   * Run a specific action card on a device.
   *
   * Device lookup uses two strategies in order of priority:
   *   1. UUID extraction from `cardId` (format: `homey:device:<uuid>:<action>`).
   *      This is the preferred strategy: robust against LLM name hallucination or
   *      character corruption (e.g. trailing spaces, U+2039 angle-quote, etc.).
   *   2. Fuzzy name match with Unicode NFC normalization + trim, as a fallback
   *      when the cardId has an unexpected format.
   *
   * @public
   * @param {string} deviceName - Device name hint (used only as fallback; may be approximate)
   ...
   * @returns {Promise<Object>} Result object with `success` flag and optional image data
   * @example
   * // Typical call from GeminiClient MCP loop:
   * const result = await adapter.runActionCard(
   *   'Riscaldamento',
   *   'homey:device:6408b9ab-a58a-4a4c-97a0-93f50be22c9c:set_smart_schedule',
   *   {}
   * );
   */
  async runActionCard(deviceName, cardId, args = {}) {
    try {
      ...
      // Strategy 1: extract device UUID directly from cardId.
      // cardId format: "homey:device:<uuid>:<actionName>"
      // This is robust against name corruption/hallucination by the LLM.
      const deviceIdMatch = cardId && cardId.match(/^homey:device:([^:]+):/);
      if (deviceIdMatch) {
        device = Object.values(devices).find(d => d.id === deviceIdMatch[1]);
      }

      // Strategy 2: fuzzy name match with Unicode normalization and trim.
      // Handles cases where Gemini appends/mutates characters (e.g. trailing
      // spaces, angle-quote U+2039, diacritics drift, etc.).
      if (!device) {
      ...
      // If action returns tokens (text/number/boolean), include them
      if (actionResult && typeof actionResult === 'object') {
        response.tokens = actionResult;
      }

      // If card has image token, fetch the device image
      if (hasImageToken) {
        this.homey.log(`[HomeyMCPAdapter] Action card has image token, fetching device image...`);

        // Wait a bit for camera driver to update the image
        await new Promise(resolve => setTimeout(resolve, 1000));

        const imageData = await this._fetchDeviceImage(device.id);

        if (imageData) {
          // Add image data with special _ prefix (for GeminiClient to recognize)
  ...
  /**
   * Get the current image from a device (camera, doorbell, etc.)
  ...
  async getDeviceImage(deviceName) {
    ...
    try {
      ...
      if (!imageData) {
        return {
          success: false,
          error: this.homey.__('prompt.error.no_device_image')
        };
      }

      // Return with special _ prefix for GeminiClient
      return {
        success: true,
        deviceName: device.name,
  ...
  /**
   * Fetch device image from Homey's ManagerImages
   * @private
  ...
  async _fetchDeviceImage(deviceId) {
    try {
      ...
      // Get device object to access images array
      const devices = await this.api.devices.getDevices();
      ...
      // Extract actual image IDs from device.images array
      // device.images is an array of objects like: { type: 'media', id: 'albumart', imageObj: { id: '...', url: '...' } }
      const deviceImageIds = device.images
        .filter(img => img.imageObj && img.imageObj.id)
        .map(img => img.imageObj.id);

      const deviceImages = Object.values(allImages).filter(img =>
        deviceImageIds.includes(img.id)
      );
      ...
      // Get most recent image (sort by lastUpdated)
      const latestImage = deviceImages.sort((a, b) =>
      ...
      // Check size limit (4MB practical limit for inline data)
      ...
      return {
        ...
        lastUpdated: latestImage.lastUpdated
      };
  ...
  async _getDeviceActionCards(device, allActionCards = null) {
    ...
    return Object.values(allActionCards).filter(card =>
      ...
      tokens: card.tokens || [] // Output tokens
  ...
  /**
   * Get available logs for a device
   * @param {string} deviceName - Name of the device
   * @returns {Promise<Object>}
   */
  async getDeviceHistory(deviceName) {
    ...
    if (!device) {
      return {
        success: false,
        error: `Device "${deviceName}" not found. Use list_devices_in_zone or list_all_devices to find the exact name.`
      };
    }
    ...
    try {
      ...
      return {
        success: false,
        error: `Failed to retrieve logs for ${deviceName}: ${error.message}`
      };
    }
  }

  /**
   * Get time-series data from a device log.
   * @public
   * @param {string} deviceName - Name of the device.
   * @param {string} logId - ID of the log to retrieve (from getDeviceHistory).
   * @param {string} resolution - Time resolution string (e.g. 'last24Hours', 'last7Days').
   *   NOTE: Log.getEntries() in HomeyAPIV3 drops the resolution argument.
   *   We call this.api.insights.getLogEntries() directly to pass the resolution.
   * @returns {Promise<Object>} Object with formatted time-series entries and statistics.
   * @example
  ...
  async getLogEntries(deviceName, logId, resolution) {
    ...
    try {
      // Retrieve all logs for the device to validate logId
      const logs = await device.getLogs();
      ...
      if (!log) {
        ...
        return {
          success: false,
          error: `Log "${logId}" not found for device "${deviceName}". Available logs: ${availableLogIds.join(', ')}. Use get_device_history first to see available logs.`
        };
      }

      // Log.getEntries() in HomeyAPIV3 ignores the resolution parameter (SDK bug).
      // We bypass it and call the manager method directly, which correctly forwards
      // resolution as a query parameter to the REST endpoint:
      //   GET /api/manager/insights/log/:uri/:id/entry?resolution=<value>
      const entries = await this.api.insights.getLogEntries({
        id: log.id,
        resolution,
      });

      // The response has a `values` array of { t: timestamp_ms, v: value } objects
      ...
      // Get Homey's timezone for timestamp conversion
      let userTimezone = 'UTC';
      try {
        userTimezone = this.homey?.clock?.getTimezone?.() || 'UTC';
      } catch (e) {
      ...
      // Format entries for Gemini with local time conversion.
      // Each raw entry: { t: unix_timestamp_ms, v: number|boolean }
      const formattedEntries = entriesArray.map(entry => {
        const utcDate = new Date(entry.t);

        // Compute local offset and shift the timestamp
      ...
      // Calculate basic statistics on the formatted entries
      ...
      return {
        ...
        statistics: stats,
        message: `Retrieved ${formattedEntries.length} data points for \n${log.title || logId} from ${deviceName}. Timestamps are in local time (${userTimezone}).`
      };
    } catch (error) {
      return {
        success: false,
        error: `Failed to retrieve log entries: ${error.message}`
      };
    }
  }
}
