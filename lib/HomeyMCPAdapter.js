 ...
 */
  ...
  async getLogEntries(deviceName, logId, resolution) {
    ...
    try {
      ...
      // Get log entries with specified resolution
      const entries = await log.getEntries({ resolution });

      // Extract the values array from the entries object
      ...
      const formattedEntries = entriesArray.map(entry => {
        ...
        return {
          timestamp: localDate.toISOString().replace('Z', ''), // Remove Z to indicate it's local time
          value: entry.v
        };
      });

      // Calculate basic statistics
      const values = formattedEntries.map(e => e.v).filter(v => v != null);
