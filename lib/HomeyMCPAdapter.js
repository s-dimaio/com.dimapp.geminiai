...
class HomeyMCPAdapter {
    ...
    try {
      ...
      switch (name) {
        ...
        case 'list_device_actions':
          return await this.listDeviceActions(args.deviceName, args.deviceId);
  ...
  async controlDevice(deviceName, capability, value, deviceId = null) {
    ...
    if (deviceId) {
      ...
      if (!device) {
        return { success: false, error: `Device with id "${deviceId}" not found. Call discover_devices to get valid device IDs.` };
      }
    } else {
      ...
      if (matches.length === 0) {
        return {
          success: false,
          error: `Device "${deviceName}" not found. You must call discover_devices first to see the exact device names, then call control_device again with the exact name from that list.`,
          required_action: 'Call discover_devices to see available devices'
        };
      }
  ...
  async getDeviceState(deviceName, deviceId = null) {
    ...
    if (deviceId) {
      ...
      const matches = Object.values(devices).filter(d => d.name.toLowerCase() === deviceName.toLowerCase());
      if (matches.length === 0) {
      ...
      if (matches.length > 1) {
        return {
          success: false,
          ambiguous: true,
          error: `Multiple devices named "${deviceName}" found. Please specify which one you mean by using its deviceId.`,
          matches: matches.map(d => ({
            name: d.name,
            id: d.id,
            zone: d.zone && zones[d.zone] ? zones[d.zone].name : 'Unknown'
  ...
  async listDevicesByClassInZone(deviceClass, zoneName) {
    ...
    try {
      ...
      const nestedZoneIds = this._getNestedZoneIds(targetZone.id, zones);
      const devices = await this.api.devices.getDevices();
   ...
   */
  async listDeviceActions(deviceName, deviceId = null) {
    if (!deviceName && !deviceId) {
      return { success: false, error: "Missing required parameter: provide 'deviceName' or 'deviceId'." };
    }
  ...
  async runActionCard(deviceName, cardId, args = {}, deviceId = null) {
    try {
      ...
      // Strategy 1: Explicit deviceId override
      if (deviceId) {
        device = Object.values(devices).find(d => d.id === deviceId);
      }

      // Strategy 2: extract UUID from cardId
      if (!device) {
      ...
      // Strategy 3: fuzzy name match with NFC normalisation
      if (!device && deviceName) {
        ...
        if (matches.length > 1) {
          return {
            success: false,
            ambiguous: true,
            error: `Multiple devices named "${deviceName}" found. Please retry by providing the specific deviceId.`,
            matches: matches.map(d => ({
              name: d.name,
              id: d.id,
              zone: d.zone && zones[d.zone] ? zones[d.zone].name : 'Unknown'
      ...
      if (!device) {
        return { success: false, error: `Device "${deviceName}" not found. Call discover_devices to get the exact device name, then retry run_action_card.` };
      }
