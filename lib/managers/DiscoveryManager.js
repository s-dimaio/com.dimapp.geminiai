  ...
  async listDevicesInZone(zoneName, includeHiddenGrouped = undefined) {
    ...
    try {
      ...
      const deviceList = devicesInZone.map(d => {
        ...
        if (!isAvailable && d.unavailableMessage) entry.unavailableMessage = d.unavailableMessage;
        return entry;
      });
  ...
  async getDeviceCountByZone(zoneName = null, includeHiddenGrouped = undefined) {
    try {
      ...
      if (zoneName) {
        ...
        const unavailableDevices = recursiveDevices
          ...
          .map(d => {
            ...
            if (d.unavailableMessage) entry.unavailableMessage = d.unavailableMessage;
            return entry;
          });
      ...
      const unavailableDevices = devicesArray
        .filter(d => d.available === false)
        .map(d => {
          const entry = {
            name: d.name,
      ...
      return { success: true, summary: allZonesSummary, unavailableDevices };
    } catch (error) {
      return { success: false, error: error.message };
    }
  }
