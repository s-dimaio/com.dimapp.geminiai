...
class FlowManager {
   ...
   * enriched with minimalist card health diagnostics (checking device/app existence and visibility).
   *
   * @public
   * @param {string} [flowId] - Unique UUID of the flow. Optional if flowName is provided.
   * @param {string} [flowName] - Exact name of the flow (case-insensitive). Optional if flowId is provided.
   * @returns {Promise<Object>} Details of the flow with cleaned cards and health parameters.
   * @example
   * const flowDetails = await flowManager.discoverFlowDetails('71f86318-107b-4c07-a766-e5094bd3440b');
   * // returns { success: true, type: 'standard', flow: { ... } }
   */
  async discoverFlowDetails(flowId, flowName) {
    ...
    try {
      ...
      const standardFlows = await this.adapter.api.flow.getFlows();
      const advancedFlows = await this.adapter.api.flow.getAdvancedFlows();
      const devices = await this.adapter.api.devices.getDevices().catch(() => ({}));
      const apps = await this.adapter.api.apps.getApps().catch(() => ({}));
      ...
      const cleaned = flowType === 'standard'
        ? this._cleanStandardFlow(flow, devices, apps)
        : this._cleanAdvancedFlow(flow, devices, apps);
  ...
  /**
   * Evaluates the health status of a flow card by checking if the referenced
   * device or app exists and determines its visibility state.
   *
   * @private
   * @param {Object} card - The raw flow card object.
   * @param {Object} devices - The map of all installed Homey devices.
   * @param {Object} apps - The map of all installed Homey apps.
   * @param {Set<string>} excludedIds - The set of device IDs that are members of a group.
   * @returns {Object|undefined} Minimalist health metadata if anomalies/hidden status detected, otherwise undefined.
   * @example
   * const health = this._getCardHealth(card, devices, apps, excludedIds);
   */
  _getCardHealth(card, devices, apps, excludedIds) {
    if (!card || !card.id) return undefined;

    let resourceUri = card.ownerUri || '';
    if (!resourceUri && typeof card.id === 'string') {
      const parts = card.id.split(':');
      if (parts.length > 1) {
        resourceUri = parts.slice(0, -1).join(':');
      }
    }
    ...
    if (resourceUri.startsWith('homey:device:')) {
      const deviceId = resourceUri.replace('homey:device:', '');
      const device = devices && devices[deviceId];
      if (!device) {
        health.error = 'device_missing';
      } else if (device.hidden === true || excludedIds.has(deviceId)) {
        health.hidden = true;
      }
    }

    // Check app reference
    if (resourceUri.startsWith('homey:app:')) {
      const appId = resourceUri.replace('homey:app:', '');
      const app = apps && apps[appId];
      if (!app) {
        health.error = 'app_missing';
      }
    }

    // Check native Homey card state if no previous device/app missing error was set
    if (!health.error && (card.broken === true || card.available === false)) {
      health.error = 'broken_card';
    }
  ...
  /**
   * Clean standard flow for model usage, appending minimalist card health diagnostics.
   *
   * @private
   * @param {Object} flow - Raw standard flow object.
   * @param {Object} devices - Map of all installed devices.
   * @param {Object} apps - Map of all installed apps.
   * @returns {Object} Cleaned standard flow ready for the model.
   * @example
   * const cleanFlow = this._cleanStandardFlow(rawFlow, devices, apps);
   */
  _cleanStandardFlow(flow, devices, apps) {
    ...
    const cleanCard = (c) => {
      ...
      // Preserve native fields if present for debug/diagnostics
      if (c.broken !== undefined) cardObj.broken = c.broken;
      if (c.available !== undefined) cardObj.available = c.available;
      if (c.disabled !== undefined) cardObj.disabled = c.disabled;
      if (c.error !== undefined) cardObj.error = c.error;

      const health = this._getCardHealth(c, devices, apps, excludedIds);
      if (health) {
        cardObj.health = health;
      }
      return cardObj;
    };
    ...
    return {
      ...
      name: flow.name,
      enabled: flow.enabled !== false,
      trigger: cleanCard(flow.trigger),
  ...
  /**
   * Clean advanced flow for model usage, appending minimalist card health diagnostics.
   *
   * @private
   * @param {Object} flow - Raw advanced flow object.
   * @param {Object} devices - Map of all installed devices.
   * @param {Object} apps - Map of all installed apps.
   * @returns {Object} Cleaned advanced flow ready for the model.
   * @example
   * const cleanFlow = this._cleanAdvancedFlow(rawFlow, devices, apps);
   */
  _cleanAdvancedFlow(flow, devices, apps) {
    const excludedIds = new Set();
    if (devices && typeof devices === 'object') {
      Object.values(devices).forEach(d => {
        const memberIds = d.settings?.deviceIds;
    ...
    if (flow.cards && typeof flow.cards === 'object') {
      Object.entries(flow.cards).forEach(([key, card]) => {
        ...
        // Preserve native fields if present for debug/diagnostics
        if (card.broken !== undefined) cardObj.broken = card.broken;
        if (card.available !== undefined) cardObj.available = card.available;
        if (card.disabled !== undefined) cardObj.disabled = card.disabled;
        if (card.error !== undefined) cardObj.error = card.error;
