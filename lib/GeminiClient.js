...
class GeminiClient {
   ...
   * After trimming, ensures the history starts with a 'user' role entry to
   * prevent INVALID_ARGUMENT errors caused by orphaned 'tool' or 'model'
   * (functionCall) entries at the beginning of the history.
  ...
  _pruneConversationHistory() {
    ...
    const afterMaxSize = this.conversationHistory.length;

    // Ensure the history starts with a 'user' role entry.
    // After a blind slice, the first entry could be a 'tool' (functionResponse)
    // or a 'model' (functionCall) without the preceding context, which causes
    // a 400 INVALID_ARGUMENT error from the Gemini API on the next session.
    // Drop all leading entries until we find a 'user' turn.
    let firstUserIdx = this.conversationHistory.findIndex(
      entry => entry.content?.role === 'user'
    );
    if (firstUserIdx > 0) {
      this.conversationHistory = this.conversationHistory.slice(firstUserIdx);
      console.log(`[GeminiClient] Pruned ${firstUserIdx} leading non-user entries to ensure valid history start`);
    } else if (firstUserIdx === -1) {
      // No user entry at all — the entire history is invalid, clear it
      this.conversationHistory = [];
      console.log('[GeminiClient] Cleared entire history: no user entry found after pruning');
    }

    if (beforeCount !== this.conversationHistory.length) {
      console.log(`[GeminiClient] Pruned conversation history: ${beforeCount} → ${this.conversationHistory.length} entries (TTL removed: ${beforeCount - afterTTL}, max-size trimmed: ${afterTTL - afterMaxSize})`);
    }
  }
  ...
  async generateTextWithMCP(prompt) {
    ...
    while (turnCount < MAX_TURNS) {
      ...
      if (turnCount >= MAX_TURNS) {
        ...
        // Clear history instead of saving an incomplete state: the last entry in
        // the chat is a 'tool' (functionResponse) with no closing 'model' text
        // turn. Preserving it would cause a 400 INVALID_ARGUMENT on the next call.
    ...
    // Clear history: same reasoning as the MAX_TURNS check above — the chat
    // ended without a closing model text turn, so the history is malformed.
    this.clearConversationHistory();
    return {
      response: this.homey.__('prompt.error.max_turns_reached'),
      success: false,
