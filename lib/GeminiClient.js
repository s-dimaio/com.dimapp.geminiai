...
class GeminiClient {
  ...
  async _buildChatConfig(modelName, cachedContentName) {
    ...
    if (cachedContentName) {
      ...
      if (this.enableGoogleSearch && !isGemini3Model) {
        console.log(`[MCP] Warning: Google Search disabled in inline config because model ${modelName} (non-Gemini-3) does not support combining Google Search with Function Calling`);
      }
  ...
  async _ensureCache(modelName) {
    ...
    const includeGoogleSearch = this.enableGoogleSearch && isGemini3Model;
    if (this.enableGoogleSearch && !isGemini3Model) {
      console.log(`[MCP] Warning: Google Search disabled in cache because model ${modelName} (non-Gemini-3) does not support combining Google Search with Function Calling`);
    }
  ...
  async generateTextWithMCP(prompt, options = {}) {
    ...
    while (turnCount < MAX_TURNS) {
      ...
      if (functionCalls.length === 0) {
        ...
        if (!flowToolUsedInSession && currentModel === this.shFlowModel) {
