...
class GeminiClient {
  constructor(apiKey, options = {}) {
    ...
    this.smartHomeModel = options.smartHomeModel || options.modelName || DEFAULT_SMART_HOME_MODEL;
   ...
   * does not support caching), the error is caught and `null` is returned. The caller
   * ({@link GeminiClient#generateTextWithMCP}) then falls back to inline mode, injecting
   * systemInstruction, tools, and toolConfig directly in the chat config instead.
   *
   * @private
   ...
   * @example
   * // Used internally by generateTextWithMCP:
   * const cacheName = await this._ensureCache();
   * if (cacheName) {
   *   // Use cached context
   * } else {
   *   // Fall back to inline systemInstruction + tools
   * }
   */
  async _ensureCache() {
    ...
    // Only Gemini 3 models support combining Google Search with custom Function Calling.
    // For older models, we must omit googleSearch if custom tools are provided.
    const includeGoogleSearch = this.enableGoogleSearch && isGemini3Model;
    if (this.enableGoogleSearch && !isGemini3Model) {
      console.log(`[MCP] Warning: Google Search disabled in cache because model ${this.smartHomeModel} (non-Gemini-3) does not support combining Google Search with Function Calling`);
    }
  ...
  async generateTextWithMCP(prompt, options = {}) {
    ...
    // Attempt to create/reuse a context cache (cached systemInstruction + tools).
    // Returns null for Free Tier API keys or unsupported models — see _ensureCache().
    ...
    if (cachedContentName) {
      ...
      // cachedContent (see _ensureCache). The API rejects any of those fields
      ...
      const includeGoogleSearch = this.enableGoogleSearch && isGemini3Model;
      if (this.enableGoogleSearch && !isGemini3Model) {
        console.log(`[MCP] Warning: Google Search disabled in inline config because model ${this.smartHomeModel} (non-Gemini-3) does not support combining Google Search with Function Calling`);
      }
    ...
    console.log(`[MCP] Chat session created — ${sessionMode} | toolConfig: ${isGemini3Model ? 'VALIDATED' : 'AUTO'}`);
