  ...
  */
  ...
  async generateTextWithMCP(prompt) {
    ...
    while (turnCount < MAX_TURNS) {
      ...
      // If we just executed any successful action, add explicit prompt for natural language response
      // This helps ensure Gemini generates a text response in AUTO mode
      if (hasInformativeAction || hasDeviceAction) {
        const promptText = hasDeviceAction 
          ? "Based on the action performed, please provide a clear confirmation message in Italian to inform the user that the operation was successful."
          : "Based on the information retrieved, please provide a clear and natural language answer to the user's question in Italian.";
        
        contents.push({
          role: "user",
          parts: [{ text: promptText }]
        });
        console.log(`[MCP] Added explicit response prompt after ${hasDeviceAction ? 'device' : 'informative'} action`);
      }
