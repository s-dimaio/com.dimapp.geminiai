...
class GeminiClient {
  ...
  */
  ...
  async generateTextWithMCP(prompt) {
    ...
    const contents = this._getConversationHistoryContents();
    console.log(`[MCP] Conversation history: ${contents.length} messages`);
    ...
    while (turnCount < MAX_TURNS) {
      ...
      }, `generateTextWithMCP (turn ${turnCount})`);
      ...
      // Also add to persistent history for cross-call memory
      this._addToConversationHistory(candidate.content);
      ...
      // Also add to persistent history for cross-call memory
      this._addToConversationHistory(functionResponseContent);
