...
class Scheduler {
    ...
    async scheduleCommand(command, executeAt, description) {
        try {
            ...
            if (delayMinutes === 0) {
                ...
                timeInfo = `${seconds} second${seconds !== 1 ? 'i' : 'o'}`;
    ...
    /**
     * Lists all currently pending scheduled commands.
     *
     * @public
     * @returns {Promise<Object[]>} Array of pending scheduled command objects.
     * @example
     * const list = await scheduler.listScheduledCommands();
     * // [ { scheduleId: '...', command: '...', executeAt: '...', description: '...', createdAt: '...' } ]
     */
    async listScheduledCommands() {
        try {
            const scheduledCommands = this.homey.settings.get('scheduled_commands') || {};
            const list = Object.entries(scheduledCommands)
            ...
            list.sort((a, b) => new Date(a.executeAt) - new Date(b.executeAt));
            return list;
        } catch (error) {
            this.homey.error('[listScheduledCommands] Error:', error);
            throw error;
        }
    }
